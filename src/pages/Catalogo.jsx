...
import { useEffect, useRef, useState } from 'react'; // 1. Importa useRef
...
const Catalogo = ({ isAdmin = false }) => {
    ...
    const fetchProductos = async () => {
      try {
        ...
        const agrupados = data.reduce((acc, producto) => {
          // La API ya nos da el nombre de la selección
  ...
  const handleSeleccionClick = (seleccion) => {
    ...
    if (!isMobile && mainContentRef.current) {
      setTimeout(() => {
        // Pequeño delay para dar tiempo a que el DOM se actualice
  ...
  return (
    <div className="catalogo-container">
      {successMessage && (
        <AlertMsg message={successMessage} type="success" isConfirm={false} show={true} />
      )}
      {isMobile && (
      ...
      <aside className={`catalogo-sidebar ${!isSidebarOpen ? 'sidebar-hidden' : ''}`}>
        <h2>Selecciones</h2>
        {isAdmin && (
          <button className="crear-producto-btn" onClick={() => navigate('/admin/crear')}>
            + Crear Producto
          </button>
        )}
        <ul className="marcas-list">
          {seleccionesOrdenadas.map((seleccion) => (
            <li
              key={seleccion}
              className={`marca-item ${seleccionSeleccionada === seleccion ? 'active' : ''}`}
            >
      ...
      <main className="catalogo-main" ref={mainContentRef}>
        {isAdmin && (
          <h1>
            <i className="fas fa-cogs"></i> - Catálogo de Productos
          </h1>
        )}
